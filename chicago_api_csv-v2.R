...
chicagocrime <- read_csv(crime_file, 
                        ...
                        ) %>% rename(datetime=date) %>% arrange(datetime)
...
title = paste("Chicago homicides year-to-date as of", format(maxdate, format="%b %d"))
...
homicide %>%  filter( year >= 2015)  %>% 
              ...
              ggplot() + aes(ydate, ysum, color = factor(year)) + 
                        geom_line() +
                        labs(title=title, x="Time of year", y="Cumulative homicide count", color="By year") +
